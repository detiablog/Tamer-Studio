...
export default function FeatureFlagsPage() {
  ...
  const handleViewHistory = (id: string) => {
    ...
    alert(`${t("admin.featureFlags.history", "History for")} "${flag?.name}":\n\n${mockHistory.map((h) => `${h.date} \u2014 ${h.action} by ${h.user}`).join("\n")}`);
  };
      ...
      <DashboardCard>
        ...
        <AdminDataTable
          ...
          columns={[
            ...
            { key: "name", header: t("common.name", "Name"), render: (f: any) => <span className="font-medium text-sm">{f.name}</span> },
            { key: "status", header: t("common.status", "Status"), render: (f: any) => <Badge tone={f.status === "Enabled" ? "success" : "muted"}>{f.status}</Badge> },
            { key: "rollout", header: t("admin.featureFlags.rollout", "Rollout"), render: (f: any) => <div className="flex items-center gap-2"><div className="h-1.5 w-16 overflow-hidden rounded-full bg-muted/40"><div className="h-full rounded-full bg-primary transition-all" style={{ width: f.rollout }} /></div><span className="text-xs">{f.rollout}</span></div> },
            { key: "lastModified", header: t("admin.featureFlags.lastModified", "Last Modified"), render: (f: any) => <span className="text-xs text-muted-foreground">{f.lastModified}</span> },
            { key: "actions", header: "", align: "right", render: (f: any) => (
              <div className="flex items-center gap-1 justify-end">
                ...
                <Button variant="ghost" size="icon-xs" onClick={() => handleViewHistory(f.id)} aria-label={t("admin.featureFlags.viewHistory", "View history")}><History className="size-3.5" /></Button>
              </div>
            )},
          ]}
        />
