...
type Tab = "templates" | "genres" | "analytics";
...
export function AdminDramaStudioPageClient() {
  const { t } = useLocalizationContext();
  const [activeTab, setActiveTab] = React.useState<Tab>("templates");

  const { data: templatesData, isLoading: templatesLoading, mutate: mutateTemplates } = useSWR("/api/admin/drama/templates", fetcher);
  const { data: genresData, isLoading: genresLoading, mutate: mutateGenres } = useSWR("/api/admin/drama/genres", fetcher);
  const { data: analyticsData, isLoading: analyticsLoading } = useSWR("/api/admin/drama/analytics", fetcher);
  ...
  const handleDeleteTemplate = async (id: string) => {
    try {
      const res = await fetch(`/api/admin/drama/templates/${id}`, { method: "DELETE" });
      if (res.ok) {
  ...
  const handleDeleteGenre = async (id: string) => {
    try {
      const res = await fetch(`/api/admin/drama/genres/${id}`, { method: "DELETE" });
      if (res.ok) {
        toast.success(t("common.success"));
        mutateGenres();
      }
            ...
            <tbody>
              {genres.map((genre: any) => (
                <tr key={genre.id} className="border-b border-border hover:bg-muted/30">
                  ...
                  <td className="py-3 px-2">{genre.projectCount ?? 0}</td>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <DashboardCard>
          <div className="flex items-center gap-2 text-sm text-muted-foreground mb-1">
            ...
            {t("dramaStudio.totalProjects")}
          </div>
          <div className="text-2xl font-semibold">{analytics.totalProjects ?? 0}</div>
        </DashboardCard>
        <DashboardCard>
          <div className="flex items-center gap-2 text-sm text-muted-foreground mb-1">
            <Clapperboard className="size-4" />
            {t("dramaStudio.totalEpisodes")}
          </div>
        ...
        <DashboardCard>
          <div className="flex items-center gap-2 text-sm text-muted-foreground mb-1">
            ...
            {t("dramaStudio.creditsUsed")}
          </div>
          <div className="text-2xl font-semibold">{analytics.creditsUsed ?? 0}</div>
        </DashboardCard>
      </div>

      <DashboardCard title={t("admin.analytics.label")} description={t("dramaStudio.description")}>
        <div className="flex items-center justify-center py-12">
  ...
  return (
    <PageContainer>
      <div className="space-y-6">
        <Breadcrumbs
          items={[
            { label: t("dramaStudio.title"), href: "/admin/drama" },
          ]}
        />
        <PageHeader
