...
export function SettingsPage({ adminToken }: SettingsPageProps) {
  ...
  const testProvider = async (id: string) => {
    setTestingId(id);
    try {
      const res = await fetch(`/api/admin/email/providers/${encodeURIComponent(id)}/test`, { method: "POST", headers: authHeaders });
      const data = await res.json();
  ...
  const deleteProvider = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/admin/email/providers/${encodeURIComponent(id)}`, { method: "DELETE", headers: authHeaders });
      const data = await res.json();
