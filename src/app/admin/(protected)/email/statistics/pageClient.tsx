...
export default function StatisticsPage({ adminToken }: StatisticsPageProps) {
  ...
  const fetchStats = () => {
    setLoading(true);
    Promise.all([
      fetch(`/api/admin/email/statistics?range=${dateRange}`, {
        headers: authHeaders,
      }).then(r => r.json()),
      fetch(`/api/admin/email/statistics/providers?range=${dateRange}`, {
        headers: authHeaders,
      }).then(r => r.json()),
