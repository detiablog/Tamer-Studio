...
export default function QueuesPage() {
  ...
  const handleRefresh = () => {
    setLoading(true);
    setTimeout(() => { setQueues(MOCK_QUEUES); setLoading(false); toast.success(t("admin.queuesRefreshed", "Queues refreshed")); }, 600);
  };

  const handleRetryFailed = (id: string) => {
    setQueues((prev) => prev.map((q) => q.id === id ? { ...q, jobsFailed: 0 } : q));
    toast.success(t("admin.retryingFailedJobs", "Retrying failed jobs"));
  };
  ...
  const handleClear = (id: string) => {
    ...
    toast.success(t("admin.queueCleared", "Queue cleared"));
  };

  const handleToggleStatus = (id: string) => {
    setQueues((prev) => prev.map((q) => q.id === id ? { ...q, status: q.status === "Active" ? "Paused" : "Active" } : q));
    toast.success(t("admin.queueStatusUpdated", "Queue status updated"));
  };
