    ...
    <div className="space-y-6">
      ...
      <DashboardCard>
        <div className="mb-6 flex items-center justify-between">
          ...
          <Button variant="outline" size="sm" onClick={() => toast.success(t("admin.subscriptions.synced", "Subscriptions synced with billing provider"))}><RefreshCw className="mr-2 size-4" />{t("common.refresh", "Refresh")}</Button>
        </div>
          ...
          columns={[
            ...
            { key: "actions", header: "", align: "right", render: (s: any) => (
              <div className="flex items-center gap-1 justify-end">
                {s.status === "Active" ? <Button variant="ghost" size="icon-xs" onClick={() => handleCancel(s.id)} className="text-destructive hover:text-destructive" aria-label={t("admin.subscriptions.cancel", "Cancel")}><ArrowUpRight className="size-3.5" />{t("admin.subscriptions.cancel", "Cancel")}</Button> : <Button variant="ghost" size="icon-xs" onClick={() => handleReactivate(s.id)} aria-label={t("admin.subscriptions.reactivate", "Reactivate")}><ArrowUpRight className="size-3.5" />{t("admin.subscriptions.reactivate", "Reactivate")}</Button>}
              </div>
            )},
          ]}
        />
