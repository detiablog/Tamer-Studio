...
function AdminLoginForm({ error, csrfToken }: { error?: string; csrfToken: string }) {
  const { t } = useLocalizationContext();

  return (
    <form action="/api/auth/admin-login" method="POST" className="space-y-4">
      <div>
        <h2 className="text-xl font-semibold">{t("admin.login.title")}</h2>
        <p className="text-xs text-muted-foreground mt-1">{t("admin.login.subtitle")}</p>
      </div>

      {error && (
        <div className="rounded-lg border border-destructive/20 bg-destructive/5 p-3 text-xs text-destructive">
          {t(`admin.login.errors.${error}`)}
        </div>
      )}

      <div className="space-y-2">
        <label htmlFor="master_key" className="text-sm font-medium">{t("admin.login.masterKeyLabel")}</label>
        <input
          id="master_key"
          name="master_key"
          type="password"
          ...
          placeholder={t("admin.login.masterKeyPlaceholder")}
        />
      </div>
      ...
      <p className="text-center text-xs text-muted-foreground">
        <a href="/" className="text-primary hover:underline">{t("admin.login.backToHome")}</a>
      </p>
    </form>
  );
}
