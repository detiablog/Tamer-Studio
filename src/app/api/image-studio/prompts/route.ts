...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const prompts = await imageStudioService.listPromptLibrary(userId, {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!body.name || !body.prompt) {
      return NextResponse.json(errorResponse("VALIDATION_ERROR", "name and prompt are required"), { status: 400 });
    }

    const prompt = await imageStudioService.createPrompt(userId, {
      name: body.name,
      prompt: body.prompt,
