...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const result = await modelRegistryService.listCapabilities();
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const capability = await modelRegistryService.createCapability({
      name: body.name,
      displayName: body.displayName,
      description: body.description,
      category: body.category,
    });
    return NextResponse.json(successResponse(capability), { status: 201 });
  } catch (error) {
    return mapErrorToResponse(error);
  }
}
