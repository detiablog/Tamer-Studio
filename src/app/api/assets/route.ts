...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!body.kind) {
      return NextResponse.json(errorResponse("VALIDATION_ERROR", "kind is required"), { status: 400 });
    }
    const item = await assetManagementService.create({
      kind: body.kind,
      ...
      tags: body.tags,
      createdBy: ctx.state.userSession!.userId,
    });
