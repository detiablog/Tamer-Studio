...
export async function GET() {
  try {
    ...
    const rows = result as unknown as Array<{ database: string; time: string }>;
    return NextResponse.json({ status: "healthy", latencyMs, database: rows[0]?.database, time: rows[0]?.time });
  } catch (error) {
