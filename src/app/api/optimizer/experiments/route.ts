...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const result = await conversionOptimizerService.listExperiments(userId, {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const experiment = await conversionOptimizerService.createExperiment(userId, {
    ...
    return NextResponse.json(successResponse(experiment), { status: 201 });
  } catch (error) {
    return mapErrorToResponse(error);
  }
}
