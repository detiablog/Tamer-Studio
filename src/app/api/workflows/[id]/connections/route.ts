...
export async function GET(
  ...
  try {
    ...
    const connections = await repo.listConnections(id);
    return NextResponse.json(successResponse(connections));
  } catch (error) {
...
export async function POST(
  ...
  try {
    ...
    return NextResponse.json(successResponse(connection), { status: 201 });
  } catch (error) {
    return mapErrorToResponse(error);
  }
}
