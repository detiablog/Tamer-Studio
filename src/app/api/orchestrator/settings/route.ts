...
import { orchestratorSettingsService } from "@/core/orchestrator/settings.service";
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const settings = await orchestratorSettingsService.getSettings(userId);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const settings = await orchestratorSettingsService.upsertSettings(userId, {
      ...
      creditWarningThreshold: body.creditWarningThreshold,
      defaultPriority: body.defaultPriority,
      allowedModules: body.allowedModules,
      metadata: body.metadata,
    });
    return NextResponse.json(successResponse(settings), { status: 201 });
  } catch (error) {
    return mapErrorToResponse(error);
  }
}
