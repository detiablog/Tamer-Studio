...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const result = await pipelineBuilderService.listExecutions(userId, {
      page: Number(searchParams.get("page")) || undefined,
      limit: Number(searchParams.get("limit")) || undefined,
      status: searchParams.get("status") || undefined,
      pipelineId: searchParams.get("pipelineId") || undefined,
    });
    return NextResponse.json(paginatedResponse(result.executions, result.total, result.page, result.limit));
  } catch (error) {
