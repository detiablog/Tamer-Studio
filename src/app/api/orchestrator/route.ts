...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const result = await pipelineBuilderService.listPipelines(userId, {
      page: Number(searchParams.get("page")) || undefined,
      limit: Number(searchParams.get("limit")) || undefined,
      status: searchParams.get("status") || undefined,
      type: searchParams.get("type") || undefined,
    });
    return NextResponse.json(paginatedResponse(result.pipelines, result.total, result.page, result.limit));
  } catch (error) {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const pipeline = await pipelineBuilderService.createPipeline(userId, {
      name: body.name,
    ...
    return NextResponse.json(successResponse(pipeline), { status: 201 });
  } catch (error) {
    return mapErrorToResponse(error);
  }
}
