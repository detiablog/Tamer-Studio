...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const result = await videoStudioService.listStoryboards(userId, {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const storyboard = await videoStudioService.createStoryboard(userId, {
      projectId: body.projectId,
      name: body.name,
      description: body.description,
      settings: body.settings,
    });
    return NextResponse.json(successResponse(storyboard), { status: 201 });
  } catch (error) {
    return mapErrorToResponse(error);
  }
}
