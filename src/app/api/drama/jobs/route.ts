...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const result = await dramaStudioService.listJobs(userId, {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!body.projectId || !body.type) {
      return NextResponse.json(errorResponse("VALIDATION_ERROR", "projectId and type are required"), { status: 400 });
    }

    const job = await dramaStudioService.createJob({
