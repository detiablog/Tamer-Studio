...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!body.conditions) {
      return NextResponse.json(errorResponse("VALIDATION_ERROR", "conditions are required"), { status: 400 });
    }

    const result = await ruleEngineService.evaluateConditions(body.conditions, body.context || {});
