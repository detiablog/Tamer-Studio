...
import { reportEngineService } from "@/core/automation/report-engine.service";
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const result = await reportEngineService.listReports(userId, {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!body.reportType) {
      return NextResponse.json(errorResponse("VALIDATION_ERROR", "reportType is required"), { status: 400 });
    }

    const report = await reportEngineService.generateReport(userId, body.reportType, body.period);
