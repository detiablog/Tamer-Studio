...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const result = await deploymentService.listDeployments({ status, environment, page, limit });
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const deployment = await deploymentService.createDeployment({
      ...
      deployedBy: body.deployedBy || ctx.state.userSession?.userId,
      notes: body.notes,
      metadata: body.metadata,
    });
    return NextResponse.json(successResponse(deployment), { status: 201 });
  } catch (error) {
    return mapErrorToResponse(error);
  }
}
