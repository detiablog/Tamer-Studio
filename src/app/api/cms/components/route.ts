...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const component = await cmsService.registerComponent(body);
    return NextResponse.json(successResponse(component, "Component registered successfully"), { status: 201 });
  } catch (error) {
    return mapErrorToResponse(error);
  }
}