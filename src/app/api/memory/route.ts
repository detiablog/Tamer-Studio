...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const result = await creativeMemoryService.listMemories(userId, {
      category: searchParams.get("category") || undefined,
      search: searchParams.get("search") || undefined,
      pinnedOnly: searchParams.get("pinnedOnly") === "true" || undefined,
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!body.category || !body.key) {
      return NextResponse.json(errorResponse("VALIDATION_ERROR", "category and key are required"), { status: 400 });
    }

    const memory = await creativeMemoryService.createMemory(userId, {
