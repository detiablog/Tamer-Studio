...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!body.memories && !body.brands && !body.preferences && !body.learningEvents) {
      return NextResponse.json(errorResponse("VALIDATION_ERROR", "At least one data section is required"), { status: 400 });
    }

    const result = await creativeMemoryService.importAll(userId, body);
