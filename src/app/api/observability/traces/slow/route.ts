...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const minDurationMs = searchParams.get("minDurationMs") ? Number(searchParams.get("minDurationMs")) : 1000;
    const hours = searchParams.get("hours") ? Number(searchParams.get("hours")) : 24;

    const traces = await obsTracingService.getSlowTraces(minDurationMs, hours);
    return NextResponse.json(successResponse(traces));
  } catch (error) {
