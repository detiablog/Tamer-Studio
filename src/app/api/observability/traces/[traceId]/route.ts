...
export async function GET(
  ...
  try {
    ...
    const spans = await obsTracingService.getTrace(traceId);
    if (!spans.length) {
      return NextResponse.json(errorResponse("NOT_FOUND", "Trace not found"), { status: 404 });
    }
    return NextResponse.json(successResponse(spans));
  } catch (error) {
