...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const dashboards = await obsDashboardService.list();
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const dashboard = await obsDashboardService.create(body);
    return NextResponse.json(successResponse(dashboard), { status: 201 });
  } catch (error) {
    return mapErrorToResponse(error);
  }
}
