...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const result = await obsAlertService.list({
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!body.ruleName || !body.category || !body.title) {
      return NextResponse.json(errorResponse("VALIDATION_ERROR", "ruleName, category, and title are required"), { status: 400 });
    }

    const alert = await obsAlertService.create(body);
