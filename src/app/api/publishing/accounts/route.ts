...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const accounts = await publishingService.listAccounts(userId);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!platform || !code) {
      return NextResponse.json(errorResponse("VALIDATION_ERROR", "platform and code are required"), { status: 400 });
    }

    const account = await publishingService.connectAccount(userId, platform, code);
