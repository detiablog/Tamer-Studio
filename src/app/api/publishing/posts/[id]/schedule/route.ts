import type { NextRequest } from "next/server";
import { NextResponse } from "next/server";
import type { RequestContext } from "@/core/middleware/types";
import { runMiddleware } from "@/core/middleware/compose";
import { userAuthentication } from "@/core/middleware";
import { mapErrorToResponse } from "@/app/api/mappers/error-mapper";
import { successResponse, errorResponse } from "@/app/api/mappers/response";
import { publishingService } from "@/core/publishing/publishing.service";

export async function POST(
  request: NextRequest,
  { params }: { params: Promise<{ id: string }> }
) {
  const ctx: RequestContext = {
    request,
    params: await params,
    state: {
      rateLimit: undefined,
      origin: undefined,
      adminSession: undefined,
      userSession: undefined,
      authError: undefined,
      permissionError: undefined,
      csrfError: undefined,
      rateLimitError: undefined,
      auditContext: undefined,
    },
    method: "POST",
    pathname: request.nextUrl.pathname,
    ip: request.headers.get("x-real-ip") || request.headers.get("x-forwarded-for")?.split(",")[0].trim() || undefined,
  };

  const middlewareError = await runMiddleware([userAuthentication()], ctx);
  if (middlewareError) return middlewareError;

  try {
    const userId = ctx.state.userSession?.userId;
    if (!userId) {
      return NextResponse.json(errorResponse("UNAUTHORIZED", "Unauthorized"), { status: 401 });
    }

    const { id } = await params;
    const post = await publishingService.getPost(id);
    if (!post) {
      return NextResponse.json(errorResponse("NOT_FOUND", "Post not found"), { status: 404 });
    }

    const body = await request.json();
    const socialAccountIds: string[] = body.socialAccountIds;
    const scheduledAt: string = body.scheduledAt;

    if (!socialAccountIds || socialAccountIds.length === 0) {
      return NextResponse.json(errorResponse("VALIDATION_ERROR", "socialAccountIds is required"), { status: 400 });
    }
    if (!scheduledAt) {
      return NextResponse.json(errorResponse("VALIDATION_ERROR", "scheduledAt is required"), { status: 400 });
    }

    const jobs = await publishingService.schedulePost(id, socialAccountIds, scheduledAt);
    return NextResponse.json(successResponse(jobs));
  } catch (error) {
    return mapErrorToResponse(error);
  }
}
