...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const result = await affiliateStudioService.listBrandKits(userId, {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const brandKit = await affiliateStudioService.createBrandKit(userId, {
      name: body.name,
      logo: body.logo,
      colors: body.colors,
      tone: body.tone,
    ...
    return NextResponse.json(successResponse(brandKit), { status: 201 });
  } catch (error) {
    return mapErrorToResponse(error);
  }
}
