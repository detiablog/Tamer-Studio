...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const result = await featureRequestService.listRequests(filters);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const feature = await featureRequestService.submitRequest(userId, {
      title: body.title,
    ...
    return NextResponse.json(successResponse(feature), { status: 201 });
  } catch (error) {
    return mapErrorToResponse(error);
  }
}
