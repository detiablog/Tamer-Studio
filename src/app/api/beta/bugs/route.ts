...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const result = await bugReportService.listBugs(filters);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const bug = await bugReportService.submitBug(userId, {
      title: body.title,
    ...
    return NextResponse.json(successResponse(bug), { status: 201 });
  } catch (error) {
    return mapErrorToResponse(error);
  }
}
