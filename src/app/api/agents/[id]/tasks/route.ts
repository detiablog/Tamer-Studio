...
export async function GET(
  ...
  try {
    ...
    const result = await agentPlatformService.listTasksByAgent(id, userId, {
...
export async function POST(
  ...
  try {
    ...
    if (!body.title) {
      return NextResponse.json(errorResponse("VALIDATION_ERROR", "title is required"), { status: 400 });
    }

    const task = await agentPlatformService.createTask(userId, {
      agentId: id,
      title: body.title,
