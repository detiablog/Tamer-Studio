...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const taxes = await pricingService.findActiveTaxes(country);
    return NextResponse.json(successResponse(taxes));
  } catch (error) {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const tax = await pricingService.createTax({
      name: body.name,
      type: body.type,
      rate: body.rate,
    ...
    return NextResponse.json(successResponse(tax), { status: 201 });
  } catch (error) {
    return mapErrorToResponse(error);
  }
}
