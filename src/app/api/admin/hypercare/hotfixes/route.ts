...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const result = await hypercareService.getHotfixes({
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const hotfix = await hypercareService.createHotfix(body, adminId);
    return NextResponse.json(successResponse(hotfix, "Hotfix created"));
  } catch (error) {
    return mapErrorToResponse(error);
  }
}
