...
export async function PATCH(
  ...
  try {
    ...
    const hotfix = await hypercareService.updateHotfix(id, body);
    if (!hotfix) {
      return NextResponse.json(
        { success: false, error: { code: "NOT_FOUND", message: "Hotfix not found" } },
        { status: 404 }
      );
    }
    return NextResponse.json(successResponse(hotfix, "Hotfix updated"));
  } catch (error) {
