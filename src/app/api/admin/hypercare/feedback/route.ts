...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const result = await hypercareService.getFeedback({
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const feedback = await hypercareService.createFeedback(body);
    return NextResponse.json(successResponse(feedback, "Feedback submitted"));
  } catch (error) {
    return mapErrorToResponse(error);
  }
}
