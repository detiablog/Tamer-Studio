...
export async function GET(request: NextRequest) {
  ...
  try {
    const exports = await biEngine.listExports();
    return NextResponse.json(successResponse(exports));
  } catch (error) {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const requestedBy = session?.adminId || "system";

    const exportRecord = await biEngine.createExport({
      name: body.name,
      reportId: body.reportId,
      format: body.format,
      requestedBy,
    });
    return NextResponse.json(successResponse(exportRecord));
  } catch (error) {
    return mapErrorToResponse(error);
  }
}
