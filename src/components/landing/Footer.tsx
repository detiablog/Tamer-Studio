...
export function Footer({ section }: SectionRendererProps) {
  ...
  const socialLinks = ((section.config.socialLinks as Array<{ label: string; href: string; ariaLabel?: string }>) || []).filter(
    (s) => s.href
  );
  const contactInfo = (section.config.contactInfo as { email?: string; supportHref?: string }) || {};
  const emailHref = contactInfo.email || "mailto:support@tamer.studio";
  const supportHref = contactInfo.supportHref || "/support";
        ...
        <div className="grid grid-cols-2 gap-8 lg:grid-cols-5 mb-12">
          ...
          {links.legal && links.legal.length > 0 && (
            <div>
              ...
              <ul className="space-y-2.5">
                <li>
                  <a href={emailHref} className="text-sm text-muted-foreground transition hover:text-primary group inline-flex items-center gap-1">
                    {t("marketing.footerContactEmail")}
                    <ArrowRight className="size-3 opacity-0 group-hover:opacity-100 group-hover:translate-x-0.5 transition" />
                  </a>
                </li>
                <li>
                  <Link href={supportHref as any} className="text-sm text-muted-foreground transition hover:text-primary group inline-flex items-center gap-1">
